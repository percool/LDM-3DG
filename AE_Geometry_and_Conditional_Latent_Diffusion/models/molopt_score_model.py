...
from models_diffusion.ddpm_conditional_pocket import Model as DDPMModel
from models_diffusion.ddpm_utils_conditional_pocket import DDPMSampler
...
class ScorePosNet3D(nn.Module):
    ...
    def __init__(self, config, protein_atom_feature_dim, ligand_atom_feature_dim):
        ...
        if self.time_emb_dim > 0:
            ...
            self.ligand_atom_emb = nn.Linear(ligand_atom_feature_dim+250, emb_dim)
        ...
        self.adapter_encoder_prot = nn.Linear(27, 249)
        self.adapter_encoder_lig = nn.Linear(13, 249)
        config['hidden_dim'] = 250
        config['n_heads'] = 10
        self.encoder_3d = get_refine_net(self.refine_net_type, config)
    
    def forward_encoder(self, protein_pos, protein_v, batch_protein, init_ligand_pos, init_ligand_v, batch_ligand,
        ...
        h_protein = self.adapter_encoder_prot(protein_v)
        init_ligand_h = self.adapter_encoder_lig(input_ligand_feat)
        ...
        outputs = self.encoder_3d(h_all, pos_all, mask_ligand, batch_all, return_all=return_all, fix_x=fix_x)

        # pdb.set_trace()

        return outputs['h_pool']
    ...
    def get_diffusion_loss(
        ...
        emb = preds[batch_ligand]

        # avoiding collapse
        preds_norm = preds.norm(dim=1)
        sim_matrix = torch.einsum('ik,jk->ij', preds, preds) / torch.einsum('i,j->ij', preds_norm, preds_norm)
        sim_matrix = torch.exp(sim_matrix)
        pos_sim = sim_matrix[range(preds.shape[0]), range(preds.shape[0])]
        ...
        preds = self(
            ...
            init_ligand_v= torch.cat([F.one_hot(ligand_v, self.num_classes).float(), emb], dim=1),
            batch_ligand=batch_ligand,
            time_step=time_step
        )

        pred_ligand_pos, pred_ligand_v = preds['pred_ligand_pos'], preds['pred_ligand_v']
        pred_ligand_v = F.one_hot(ligand_v, self.num_classes).float()
        ...
        loss_v = 0
        loss = loss_pos + loss_neg_contrast * 0.1
    ...
    def sample_diffusion(self, protein_pos, protein_v, batch_protein,
                         init_ligand_pos, real_ligand_pos, init_ligand_v, batch_ligand,
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            preds = self.forward_encoder(
                ...
                init_ligand_pos=real_ligand_pos,
                init_ligand_v=ligand_v,
                batch_ligand=batch_ligand,
                time_step=t
            )
            ...
            preds = self(
                ...
                init_ligand_pos=ligand_pos,
                init_ligand_v=torch.cat([F.one_hot(ligand_v, self.num_classes).float(), emb], dim=1), # ligand_v,
                batch_ligand=batch_ligand,
                time_step=t
            )
    ...
    @torch.no_grad()
    def reconstruc_3d(self, protein_pos, protein_v, batch_protein,
                         init_ligand_pos, emb_lat, init_ligand_v, batch_ligand,
                         num_steps=None, center_pos_mode=None, pos_only=False):
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            emb = emb_lat[batch_ligand]

            lig_v = torch.zeros((ligand_v.shape[0], self.num_classes)).cuda()
            lll = lig_v[ligand_v<self.num_classes] ; lll[torch.arange((ligand_v<self.num_classes).sum()), ligand_v[ligand_v<self.num_classes]] = 1
            lig_v[ligand_v<self.num_classes] = lll
            lig_v[ligand_v>=self.num_classes] = 1 / self.num_classes
            ...
            preds = self(
                ...
                init_ligand_pos=ligand_pos,
                init_ligand_v=torch.cat([lig_v, emb], dim=1), # ligand_v,
                batch_ligand=batch_ligand,
                time_step=t
            )
...
class LDM_Cond(nn.Module):
    ...
    def sample_z(self, protein_pos, protein_v, batch_protein, num_sample):
        edge_index = radius_graph(protein_pos, r=10, batch=batch_protein)
        _, emb_protein = self.encoder_3d(protein_v, protein_pos, edge_index, batch_protein, tanh=False)

        sampler = DDPMSampler(1e-4, 0.02, 500, self.ddpm, 'cuda', (500,))
        _emb_protein = emb_protein.unsqueeze(dim=1).repeat((1, num_sample, 1)).reshape((emb_protein.shape[0]*num_sample, -1))
        samples = sampler.sampling(_emb_protein, True).reshape((emb_protein.shape[0], num_sample, -1))
