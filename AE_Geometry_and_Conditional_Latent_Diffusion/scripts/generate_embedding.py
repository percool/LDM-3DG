...
if __name__ == '__main__':
    ...
    for idx in tqdm(range(len(test_set))):
        ...
        emb_2d.append( smi2emb_dict[smi].unsqueeze(dim=0) )

        
        # 3d embedding
        with torch.no_grad():
            emb_3d.append( model.forward_encoder(protein_pos=data.protein_pos.to(args.device),
                                                 protein_v=data.protein_atom_feature.float().to(args.device),
                                                 batch_protein=torch.zeros(data.protein_pos.shape[0], dtype=torch.int64).to(args.device),
                                                 init_ligand_pos=data.ligand_pos.to(args.device),
